  ...
  function pipeFailRetry(jqXHR, opts) {
    ...
    return function(input, status, msg) {
      ...
      function nextRequest() {
        $.ajax(ajaxOptions)
          .retry({times: times - 1, timeout: opts.timeout, statusCodes: opts.statusCodes, error: opts.error, start: opts.start})
          .pipe(output.resolve, output.reject);
      }

      console.log("Got status code: " + input.status + " other status " + status);
      ...
      if (times > 1 && (!jqXHR.statusCodes || $.inArray(input.status, jqXHR.statusCodes) > -1)) {
        ...
        opts.error(input, ajaxOptions, msg);
